...
BREAKER = Breaker()
ROUTES = {
    "/call": lambda payload: BREAKER.call(payload["ok"], payload["at"]),
    "/tick": lambda payload: BREAKER.tick(payload["now"]),
    "/recover": lambda payload: BREAKER.restore(),
}


class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.reply(BREAKER.stats())
